        ...
        <aside class="bg-primary-50 flex h-screen w-64 flex-col justify-between p-4 shrink-0 border-r border-primary-200/50" id="sidebar">
            <div class="space-y-6">
                ...
                <nav id="sidebar-items-container">
                    <div class="px-3 mb-2 text-xs font-semibold uppercase tracking-wider text-base-content/50">ICT Operations</div>
                    <ul class="space-y-1">
                        <!-- Assigned / Active Queue -->
                        <li>
                            <a wire:navigate href="{{ route('ict-dashboard') }}"
                               class="flex items-center justify-between rounded-btn px-4 py-2.5 text-sm font-medium transition-all {{ request()->routeIs('ict-dashboard') ? 'bg-primary text-primary-content shadow-sm' : 'hover:bg-primary-100 text-base-content/80' }}">
                        ...
                        <li>
                            <a wire:navigate href="{{ route("manage-desk") }}"
                               class="flex items-center gap-3 rounded-btn active:bg-primary-600 px-4 py-2.5 text-sm font-medium transition-all {{ request()->routeIs('manage-desk') ? 'bg-primary text-primary-content shadow-sm' : 'hover:bg-primary-100 text-base-content/80' }}">
                        ...
                        <li>
                            <a wire:navigate href="{{ route('manage-asset') }}"
                               class="flex items-center gap-3 rounded-btn px-4 py-2.5 text-sm font-medium transition-all {{ request()->routeIs('manage-asset') ? 'bg-primary text-primary-content shadow-sm' : 'hover:bg-primary-100 text-base-content/80' }}">
            ...
            <div class="border-primary-200/60 border-t pt-4 space-y-3">
                <div class="flex items-center gap-3 px-2 py-1 bg-primary-100/60 rounded-btn">
                    ...
                    <div class="overflow-hidden">
                        <p class="text-xs font-bold truncate">{{ auth()->guard('support')->user()->first_name . " " . auth()->guard('support')->user()->last_name ?? 'ICT Support Officer' }}</p>
